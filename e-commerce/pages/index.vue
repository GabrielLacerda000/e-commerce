...
<template>
    <div>
      ...
      <div v-if="error" class="text-center text-red-500">Erro ao carregar produtos: {{ error.message }}</div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div v-for="product in products" :key="product.id" class="border rounded-lg p-4 shadow hover:shadow-lg transition">
          ...
          <NuxtLink :to="`/product/${product.id}`" class="text-lg text-blue-700 font-bold hover:text-blue-500 transition">{{ product.name }}</NuxtLink>
